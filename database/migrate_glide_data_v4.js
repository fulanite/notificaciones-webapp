...
// Configuration
const config = {
    usersFile: path.join(__dirname, 'users_v3.csv'), // Optional, skip if missing
    cedulasFile: path.join(__dirname, '3d3ff9.cedulas.csv'),
    visitasFile: path.join(__dirname, 'd9f0ca.visitas.csv'),
    outputDir: __dirname,
};
...
async function migrate() {
    try {
        ...
        fs.writeFileSync(path.join(config.outputDir, 'import_usuarios_v4.sql'), usersSql.join('\n'));
        ...
        notifSql.push('ALTER TABLE notificaciones MODIFY COLUMN estado VARCHAR(50);');
        ...
        cedulas.forEach(c => {
            ...
            let asignadoUuid = asignadoRaw && userMap.has(asignadoRaw) ? userMap.get(asignadoRaw) : (asignadoRaw || 'NULL');
        ...
        fs.writeFileSync(path.join(config.outputDir, 'import_notificaciones_v4.sql'), notifSql.join('\n'));
        ...
        visitas.forEach(v => {
            ...
            let ujierUuid = ujierRaw && userMap.has(ujierRaw) ? userMap.get(ujierRaw) : (ujierRaw || 'NULL');
        ...
        fs.writeFileSync(path.join(config.outputDir, 'import_visitas_v4.sql'), visitasSql.join('\n'));
